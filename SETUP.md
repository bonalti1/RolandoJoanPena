...
app + PWA. The serverless functions (Ask AI, link unfurl, product search) are
**not** included in a drop deploy — the app still runs, those features just fall
...
2. Add environment variables under **Site settings → Environment variables**:
   - `OPENAI_API_KEY` — enables the "Ask AI" assistant
   - `OPENAI_MODEL` — optional, defaults to `gpt-4o-mini`
   - `RAPIDAPI_KEY` — optional, enables live Wishlist product search
